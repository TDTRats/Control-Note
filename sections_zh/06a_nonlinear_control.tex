...
对于跟踪参考 $x_d(t)$ 的二阶系统，误差 $e = x - x_d$，滑模面的一个自然选择是：
\begin{equation}
s = \dot{e} + \lambda e
\end{equation}
...
上一章中我们看到线性 MPC 在每个时间步求解一个二次规划（QP），使用线性模型 $\mathbf{x}_{k+1} = A_d \mathbf{x}_k + B_d \mathbf{u}_k$。这在单个工作点附近表现良好。\textbf{非线性 MPC} 将同样的滚动时域思想扩展到完整非线性动力学：
\begin{equation}
