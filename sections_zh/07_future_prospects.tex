...
\textbf{近期进展（2024--2025）：}
\begin{itemize}[nosep]
    \item 可逆 Koopman 网络使用可逆神经网络同时学习提升映射及其逆，解决了从提升坐标重构真实状态这一长期难题。
    \item 基于 Koopman 的 MPC + 控制屏障函数，将非线性安全关键控制问题化为提升空间中的简单 QP。
    \item 硬件验证案例包括四旋翼 SE(3) Koopman-MPC 和自动驾驶中的实时轮胎动力学建模。
\end{itemize}
...
\begin{enumerate}[nosep]
    \item \textbf{无模型 RL}（\href{https://arxiv.org/abs/1707.06347}{PPO}、\href{https://arxiv.org/abs/1801.01290}{SAC} 等）：通过试错直接学习策略，不需要动力学模型。通用性强，但\emph{样本效率}极低------通常需要数百万次交互，仅在仿真中可行，部署时需进行 sim-to-real 迁移。
    \item \textbf{基于模型的 RL}（\href{https://arxiv.org/abs/1906.08253}{MBPO}、\href{https://arxiv.org/abs/1912.01603}{Dreamer} 等）：先从数据中学习动力学模型，再利用该模型进行规划或策略优化。样本效率更高，更接近传统控制的思路。
\end{enumerate}
...
RL 在科研领域取得了显著成果：
\begin{itemize}[nosep]
    \item \textbf{足式运动}：MIT Mini Cheetah（\href{https://arxiv.org/abs/2208.07855}{Margolis et al., 2022}）、ANYmal（\href{https://arxiv.org/abs/2109.11978}{Miki et al., 2022}）等四足机器人通过 RL 学会了在崎岖地形上行走和奔跑。
    \item \textbf{灵巧操控}：OpenAI 的魔方求解、谷歌的机器人抓取。
    \item \textbf{无人机竞速}：苏黎世联邦理工 Swift 系统在无人机竞速中超越人类冠军（\href{https://arxiv.org/abs/2310.08984}{Kaufmann et al., 2023}）。
\end{itemize}
...
\textbf{对机器人的意义：}
\begin{itemize}[nosep]
    \item 在 8+ 个电机控制器共享的 CAN 总线上，减少不必要的报文直接改善高优先级环路的最差延迟。
    \item 在电池供电系统（无人机、移动机器人）上，更少的计算意味着更长的续航。
    \item 在多智能体系统（无人机编队、多机器人仓库）中，事件触发通信大幅降低网络拥塞。
\end{itemize}
