...
\textbf{PID 最凶险的 Bug：正反馈}

\vspace{0.3em}
在往下走之前，我们必须聊一个每支竞赛队都踩过的坑——踩上去的后果往往是电机瞬间跑飞、输出打满，机构直接撞废。

场景很简单：你写 PID 的时候，不小心把 \texttt{error = measurement - setpoint} 写反了（正确写法是 \texttt{setpoint - measurement}）。也可能不是代码的问题——电机的转向和你以为的相反，或者编码器计数方向反了。无论原因是什么，结果都一样：误差越大，控制输出不是在纠正它，而是在\textit{帮它变得更大}。这就是正反馈。

正反馈的系统不会像"调得不好的 PID"那样慢慢振荡、超调一点。它\textbf{根本不会收敛}——输出在几毫秒内直奔执行器极限，电机以最大力矩朝错误方向死转。如果没有限位保护，轻则齿轮打齿，重则结构件报废。

下图把正确的 PID（蓝色）和符号反了的 PID（红色）放在同一个被控对象上做对比。注意看：红色曲线和"参数调大了"产生的振荡完全不是一回事——它是定性地不同的行为。\textbf{再怎么调增益都救不了符号错误}，只有把符号改对才行。

\textbf{怎么自检？}每次闭环之前，花 10 秒钟做一个测试：给一个小的正向设定值阶跃，观察控制输出是否驱动被控对象朝\textit{减小}误差的方向运动。如果反了，翻转符号。养成这个习惯，能省掉无数次深夜对着失控的电机百思不得其解的痛苦。
\end{mdframed}

\begin{figure}[H]
\centering
\includegraphics[width=\textwidth]{positive_feedback_pid.pdf}
\caption{正确 PID（蓝色）平稳跟踪设定值；符号错误的 PID（红色）形成正反馈——输出瞬间冲到执行器极限，电机失控。这不是调参能解决的问题，只有改对符号才行。}
\end{figure}
