...
\textbf{Derivation sketch.} The ZOH discretization computes $G(z) = (1 - z^{-1})\,\mathcal{Z}\!\left\{\frac{G(s)}{s}\right\}$. The continuous-time step response of $G(s)$ is $y(t) = 1 - e^{-at}$; sampling at $t = nT_s$ gives $y[n] = 1 - e^{-anT_s}$. Taking the $\mathcal{Z}$-transform and multiplying by $(1 - z^{-1})$ yields the result above.
...
\textbf{Intuition.} The bilinear transform maps the entire continuous frequency axis $\omega \in [0, \infty)$ onto a finite arc of the unit circle $\omega_d \in [0, \pi/T_s)$, compressing high frequencies. The exact relationship is $\omega = \frac{2}{T_s}\tan\frac{\omega_d T_s}{2}$. Pre-warping inverts this mapping: we feed the bilinear transform a ``stretched'' frequency so that our target frequency $\omega_c$ lands at exactly the right place after the compression.
