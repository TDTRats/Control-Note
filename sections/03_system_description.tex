...
Let us sketch the derivation. From the simplified equation ($L \approx 0$), Ohm's law gives $I = (V - K_e \omega)/R$. Substituting into the mechanical equation $J\frac{d\omega}{dt} = K_t I - b\omega$:
\[
J\frac{d\omega}{dt} = \frac{K_t}{R}(V - K_e \omega) - b\omega
\]
Taking the Laplace transform (zero initial conditions): $(Js + b + K_t K_e / R)\,\Omega(s) = (K_t / R)\,V(s)$, hence:
...
\textbf{Why transfer functions are great.}\;They are compact---one fraction describes the entire system. Cascading systems is just multiplication: $G_{\text{total}}(s) = G_1(s) \cdot G_2(s)$. Stability analysis is straightforward: check the roots of the denominator (poles). And the frequency response is obtained by simply substituting $s = j\omega$.

\textbf{Why transfer functions are limited.}\;They only work for linear, time-invariant (LTI) systems. They assume zero initial conditions. They are naturally SISO---extending to MIMO requires transfer function matrices, which get unwieldy. And they hide the internal states of the system. This last point is why we need the state-space representation.
...
To derive this, we take the Laplace transform of the step response: $Y(s) = \frac{\omega_n^2}{s(s^2 + 2\zeta\omega_n s + \omega_n^2)}$. Partial fraction expansion with complex poles at $s = -\zeta\omega_n \pm j\omega_d$ (where $\omega_d = \omega_n\sqrt{1-\zeta^2}$) and inverse Laplace transform yields the result. The overshoot occurs at the first peak, $t_p = \pi/\omega_d$; substituting into $y(t)$ gives $M_p$.
...
Natural frequency: $\omega_n = \sqrt{k/m}$. \quad Damping ratio: $\zeta = \frac{b}{2\sqrt{km}}$. These follow from matching coefficients: $ms^2 + bs + k = m(s^2 + 2\zeta\omega_n s + \omega_n^2)$ gives $\omega_n^2 = k/m$ and $2\zeta\omega_n = b/m$, hence $\zeta = b/(2m\omega_n) = b/(2\sqrt{km})$.
