...
The nested block diagram looks like this. The position controller outputs a velocity setpoint. The velocity controller outputs a current (torque) command. For CAN-based ESCs (C610/C620), this current command is sent over CAN to the ESC, which handles the internal current loop and FOC commutation. For simpler drivers, the current controller outputs a voltage (PWM duty). Either way, each inner loop ``wraps around'' its own plant dynamics, taming them before the outer loop ever sees them.
...
\textbf{Priority scheduling.} When the current loop runs on your MCU (simple H-bridge drivers), it is the highest priority. When using CAN-based ESCs (C610/C620), the current loop runs inside the ESC --- so on your MCU, the CAN TX/RX and velocity loop become the highest-priority tasks. The position loop and planner are lowest priority because they change slowly and can tolerate occasional jitter.
...
\begin{enumerate}[nosep]
    ...
    \item \textbf{Current PI + FOC} runs at $\sim$10~kHz inside the C620 ESC. Your MCU sends $i_{\text{cmd}}$ as a 16-bit signed value over CAN; the ESC handles the current loop, field-oriented control, and three-phase commutation internally. It reports back the actual current, rotor angle, and velocity over CAN.
\end{enumerate}
