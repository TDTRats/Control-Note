...
\textbf{Recent progress (2024--2025):}
\begin{itemize}[nosep]
    \item Invertible Koopman Networks use invertible neural networks to learn both the lifting map and its inverse, solving the long-standing problem of reconstructing the true state from the lifted coordinates.
    \item Koopman-based MPC with Control Barrier Functions formulates safety-critical nonlinear control as a simple QP in the lifted space.
    \item Hardware demonstrations include SE(3) Koopman-MPC on quadrotor UAVs and real-time tire-dynamics modeling for autonomous vehicles.
\end{itemize}
...
\begin{enumerate}[nosep]
    \item \textbf{Model-free RL} (\href{https://arxiv.org/abs/1707.06347}{PPO}, \href{https://arxiv.org/abs/1801.01290}{SAC}, etc.): learns policies through trial and error without a dynamics model. Highly general, but \emph{sample efficiency} is extremely low---typically requiring millions of interactions, feasible only in simulation, with sim-to-real transfer needed for deployment.
    \item \textbf{Model-based RL} (\href{https://arxiv.org/abs/1906.08253}{MBPO}, \href{https://arxiv.org/abs/1912.01603}{Dreamer}, etc.): first learns a dynamics model from data, then uses it for planning or policy optimization. More sample-efficient and closer in spirit to classical control.
\end{enumerate}
...
\begin{itemize}[nosep]
    \item \textbf{Legged locomotion}: MIT Mini Cheetah (\href{https://arxiv.org/abs/2208.07855}{Margolis et al., 2022}), ANYmal (\href{https://arxiv.org/abs/2109.11978}{Miki et al., 2022}), and other quadrupeds have learned to walk and run on rough terrain via RL.
    \item \textbf{Dexterous manipulation}: OpenAI's Rubik's cube solving, Google's robotic grasping.
    \item \textbf{Drone racing}: ETH Z\"urich's Swift system beat human champions in drone racing (\href{https://arxiv.org/abs/2310.08984}{Kaufmann et al., 2023}).
\end{itemize}
...
\textbf{Why this matters for robotics:}
\begin{itemize}[nosep]
    \item On a CAN bus shared among 8+ motor controllers, reducing unnecessary messages directly improves worst-case latency for high-priority loops.
    \item On battery-powered systems (drones, mobile robots), fewer computations mean longer runtime.
    \item In multi-agent systems (UAV swarms, multi-robot warehouses), event-triggered communication drastically reduces network congestion.
\end{itemize}
