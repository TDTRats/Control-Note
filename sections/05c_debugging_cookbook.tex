...
\textbf{Step 1:} Disconnect the controller. Command a constant open-loop torque (e.g., send a fixed current command over CAN for C610/C620, or a constant PWM duty for simpler drivers). Observe the motor.
\begin{itemize}
...
\textbf{Command verification.} For CAN-based systems (C610/C620), use a CAN debugger or logic analyzer to decode the frames on the bus. Verify: (1) your MCU is transmitting at the expected rate (typically 1~kHz), (2) the CAN ID matches the ESC's expected ID, (3) the current command bytes contain the values you intend. For PWM-based drivers, put an oscilloscope probe on the motor phase output and verify the duty cycle matches your software command.
