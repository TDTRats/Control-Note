...
Every other chapter teaches you how to design controllers; this one teaches you what to do when they do not work. During the preparation phase, you will come back to this chapter repeatedly: what to check when the motor oscillates, which parameter to adjust when overshoot is too large, how to handle a runaway integral term. Master these patterns during practice, and you will react quickly when it matters.

Debugging skill is not talent---it is practice. The difference between an experienced engineer and a frustrated beginner is not that their robots have no bugs---every real system has bugs---but the speed at which they go from ``something is wrong'' to ``I found the root cause and fixed it.'' This chapter gives you that methodology.
