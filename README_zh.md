...
| # | 章节 | 主要内容 |
|---|------|----------|
| - | **两个支配世界的数：$e$ 与 $\pi$** | $\pi$ 主导周期性，$e$ 主导衰减；欧拉公式统一两者 |
| 1 | **引言** | 写作动机、学习路径、阅读方式 |
| 2 | **数字信号处理** | 频域基础、拉普拉斯/傅里叶变换、极零点入门、低通/带通/高通/陷波滤波、Z 变换 |
| 2b | **传感器标定与选型** | 传感器误差模型、IMU 六面标定、Allan 方差、编码器速度估计、磁力计标定 |
| 3 | **系统描述** | 传递函数、状态空间、稳定性、二阶系统、直流电机建模（有刷 & 无刷 FOC） |
| 4 | **经典控制理论** | 反馈基础、Bode 图、PID（单环/串级/并联）、调参方法、前馈、系统辨识 |
| 4b | **多环架构设计** | 完整控制栈、串级架构、带宽分离、状态机、定时/RTOS |
| 5 | **离散化与实现** | ZOH、Tustin、匹配 Z 变换、FIR/IIR 滤波器设计、定点数、嵌入式实现 |
| 5b | **仿真工作流** | 模型/软件/硬件在环仿真、Python 仿真模板、仿真-现实差距 |
| 5c | **调试与调参手册** | 诊断流程图、故障模式特征、真实硬件 PID 调参、比赛日应急修复 |
| 6 | **现代控制理论** | 可控性/可观性、LQR、MPC、TinyMPC、龙伯格观测器、卡尔曼/EKF、LQG、轨迹规划 |
| 6a | **非线性控制基础** | 反馈线性化、滑模控制、增益调度、Lyapunov 设计、能量整形 |
| 6b | **姿态与旋转表示** | 坐标系（NWU）、旋转矩阵、欧拉角、万向锁、轴角表示、四元数 |
| 7 | **展望** | 数据驱动控制、Koopman 算子、自适应控制、强化学习、扩散策略、VLA、Neural ODE、事件触发控制 |
